...
$canOrder = $user->authorise('core.edit.state', 'com_turnering');
$saveOrder = $listOrder == 'a.ordering';
$input = JFactory::getApplication()->input;
jimport('joomla.form.form');
JForm::addFormPath(JPATH_COMPONENT . '/models/forms');
$form = JForm::getInstance('com_turnering.kreds', 'kreds');
?>

<form action="<?php echo JRoute::_('index.php?option=com_turnering&view=kredse'); ?>" method="post" name="adminForm" id="adminForm">
                ...
                <?php if (isset($this->items[0]->ordering)) : ?>
                    <th width="10%">
                        ...
                        <?php if ($canOrder && $saveOrder) : ?>
                            <?php echo JHtml::_('grid.order', $this->items, 'filesave.png', 'kredse.saveorder'); ?>
                        <?php endif; ?>
                    </th>
                <?php endif; ?>
        ...
        <tbody>
            ...
            foreach ($this->items as $i => $item) :
                ...
                <tr class="row<?php echo $i % 2; ?>">
					...
					<?php endif; ?>
					<?php if ($canEdit): ?>
						<a href="<?php echo JRoute::_('index.php?option=com_turnering&task=kreds.edit&id=' . (int) $item->id); ?>">
                    ...
                    <?php if (isset($this->items[0]->state)) { ?>
                        <td class="center">
                            <?php echo JHtml::_('jgrid.published', $item->state, $i, 'kredse.', $canChange, 'cb'); ?>
                        </td>
                    <?php } ?>
                    ...
                    <?php if (isset($this->items[0]->ordering)) { ?>
                        <td class="order">
                            <?php if ($canChange) : ?>
                                <?php if ($saveOrder) : ?>
                                    <?php if ($listDirn == 'asc') : ?>
                                        <span><?php echo $this->pagination->orderUpIcon($i, true, 'kredse.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
                                        <span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, true, 'kredse.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
                                    <?php elseif ($listDirn == 'desc') : ?>
                                        <span><?php echo $this->pagination->orderUpIcon($i, true, 'kredse.orderdown', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
                                        <span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, true, 'kredse.orderup', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
                                    <?php endif; ?>
                                <?php endif; ?>
